...
FROM countries_world
GROUP BY Country
HAVING COUNT(*) > 1;

-- Deleting duplicate 'American Samoa'

DELETE  TOP (1) FROM countries_world
WHERE Country = 'American Samoa';



--  to check if the entire row is duplicate:

SELECT Country, Country_Code, Region, Population, Area_sq_mi, COUNT(*) AS dup_count
FROM countries_world
GROUP BY Country, Country_Code, Region, Population, Area_sq_mi
HAVING COUNT(*) > 1;
